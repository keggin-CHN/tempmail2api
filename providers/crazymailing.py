"""Crazymailing.com provider — Fake_trash_mail pattern."""
...
class CrazymailingClient(TempMailClient):
    """Client for https://www.crazymailing.com/"""

    BASE_URL = "https://www.crazymailing.com"
    ...
    def generate_email(self, duration_minutes: int = 10, domain: Optional[str] = None) -> TempEmail:
        ...
        selected = domain if domain and domain in domains else (domains[0] if domains else "crazymailing.com")
        name = ''.join(random.choices(string.ascii_lowercase + string.digits, k=10))
        address = f"{name}@{selected}"
        self._session.post(f"{self.BASE_URL}/messages", data={
            "_token": self._token,
        }, timeout=self._timeout)
        return TempEmail(address=address, provider="crazymailing")
    ...
    def get_email_detail(self, address: str, email_id: str) -> Optional[InboxEmail]:
        ...
        if resp.status_code == 200:
            return InboxEmail(id=email_id, provider="crazymailing", body_html=resp.text)
        return None
