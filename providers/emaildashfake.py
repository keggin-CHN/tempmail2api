"""Email-fake.com provider — HTML scraping with BeautifulSoup (Generatoremail pattern)."""
...
class EmaildashfakeClient(TempMailClient):
    """Client for https://email-fake.com/"""

    BASE_URL = "https://email-fake.com"
    ...
    def _get_domains(self) -> list:
        if BeautifulSoup is None:
            return ["tmpeml.com"]
        resp = self._session.get(f"{self.BASE_URL}/", timeout=self._timeout)
    ...
    def generate_email(self, duration_minutes: int = 10, domain: Optional[str] = None) -> TempEmail:
        ...
        return TempEmail(address=f"{name}@{selected}", provider="email-fake")
    ...
    def get_email_detail(self, address: str, email_id: str) -> Optional[InboxEmail]:
        ...
        if resp.status_code == 200:
            ...
            return InboxEmail(id=email_id, provider="email-fake", body_html=html)
        return None
