"""Tempmailso.com provider — Fake_trash_mail pattern."""
...
class TempmailsoClient(TempMailClient):
    """Client for https://tempmailso.com/"""

    BASE_URL = "https://tempmailso.com"
    ...
    def generate_email(self, duration_minutes: int = 10, domain: Optional[str] = None) -> TempEmail:
        ...
        selected = domain if domain and domain in domains else (domains[0] if domains else "tempmailso.com")
        name = ''.join(random.choices(string.ascii_lowercase + string.digits, k=10))
        address = f"{name}@{selected}"
        self._session.post(f"{self.BASE_URL}/messages", data={"_token": self._token}, timeout=self._timeout)
        self._session.post(f"{self.BASE_URL}/create", data={
            "_token": self._token, "name": name, "domain": selected,
        }, timeout=self._timeout)
        self._session.post(f"{self.BASE_URL}/messages", data={"_token": self._token}, timeout=self._timeout)
        return TempEmail(address=address, provider="tempmailso")
    ...
    def get_email_detail(self, address: str, email_id: str) -> Optional[InboxEmail]:
        ...
        if resp.status_code == 200:
            return InboxEmail(id=email_id, provider="tempmailso", body_html=resp.text)
        return None
